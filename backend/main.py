...
from backend.utils.text_llm import (create_poem, decompose_user_text,
                                    ...
                                    text_to_image)
from backend.utils.twitter import post_tweet_with_image_from_url
...
async def create_tweet(request: TweetRequest):
    try:
        result = post_tweet_with_image_from_url(request.caption, request.image_url)
        if result:
            return {
                "success": True,
                "tweet_id": result.get('data', {}).get('id'),
                "tweet_url": f"https://twitter.com/user/status/{result.get('data', {}).get('id')}"
            }
